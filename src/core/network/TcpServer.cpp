...
void TcpServer::start_accept()
{
	if(type == GAME_SERVER_SSL || type == GAME_CLIENT_SSL)
	{
		new_ssl_connection.reset(new TcpConnectionSSL(GET_IO_SERVICE(acceptor_), type, context_, db));
		acceptor_.async_accept(	new_ssl_connection->socket(),
								boost::bind(&TcpServer::handle_acceptSSL, this, asio::placeholders::error));
	}
	else
	{
		new_connection.reset(new TcpConnection(GET_IO_SERVICE(acceptor_), type, db));
