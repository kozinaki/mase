...
//fsys//
			if(strcmp(packet_type, "fsys") == 0)
			{
				...
				else if(txn.compare("MemCheck") == 0)
				{
					...
					{
						ping_timer = new asio::deadline_timer(GET_IO_SERVICE(socket_));//, posix_time::seconds(150)); // only create the object here, it'll be updated below anyway
						memcheck_timer = new asio::deadline_timer(GET_IO_SERVICE(socket_), posix_time::seconds(500));
