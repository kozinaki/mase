...
void TcpConnection::start()
{
	...
	if(fw->addConnection(type))
	{
		...
		ping_timer = new asio::deadline_timer(GET_IO_SERVICE(socket_));
	}
	else if(type != MISC)
...
void TcpConnection::handle_read(const system::error_code& error, size_t bytes_transferred)
{
	if (!error)
	{
		...
		if(state != SKIP && outgoingQueue.front()->isDelayed())
		{
			...
			if(!delayed_timer)
				delayed_timer = new asio::deadline_timer(GET_IO_SERVICE(socket_), posix_time::seconds(delayed_packet->getDelayTime()));
			else
